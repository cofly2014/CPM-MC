...
# Clip ResidualAttentionBlock以及VisionTransformer中使用
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        '''
        图像特征提取可以选用ModifiedResNet或VisualTransformer。
        ModifiedResNet中使用的核心优化包括：
        Zhang,2019中的anti-alias改进方法；
        使用weight norm替代了batch norm；
        GELU激活函数。
        '''
        ...
        #add by guofei
        self.return_intermediate_text_feature = False
    ...
    #CLIP的视觉编码器V来生成输入视频帧的特征
    def encode_image(self, image):
        return self.visual(image.type(self.dtype))
    #采用文本编码器T来提取相应类自然语言描述的文本嵌入
    def encode_text(self, text):
        ...
        # text.argmax(dim=-1)  64 77所以其结果为 64  每个元素为 77中的最大的那个元素的下标
        return x
    def encode_text_eh(self, xlight, text):
...
def build_model(state_dict: dict, spatial=False):
    ...
    #model里的encode_image 即：内部的self.visual 为视觉编码器。 encode_text为文本编码器
